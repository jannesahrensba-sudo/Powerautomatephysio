...
**Neu gegenüber V2:** Textbausteine und eine Längenprüfung in der
Dokumentation – siehe „Schreibhilfen" weiter unten. Der Rest ist unverändert;
wer V2 schon eingefügt hat, kann V2 im Studio löschen und V3 einfügen.
...
| **Dokumentation** | Durchgeführte Maßnahmen, Reaktion und Ergebnis, freiwillig Heimübungen. Unter den beiden Pflichtfeldern je eine Bausteinliste und ein Längenhinweis. „Behandlung speichern". |
...
| In beide Pflichtfelder `ok` schreiben, speichern | Hinweis „Bitte etwas ausführlicher: mindestens 10 Zeichen je Pflichtfeld." |
| In der Liste unter „Maßnahmen" einen Baustein wählen | Text wird angehängt, die Liste springt auf „Baustein einfügen ..." zurück |
| Noch einen Baustein wählen | Wird mit Komma angehängt, der vorherige bleibt stehen |
...
## Schreibhilfen in der Dokumentation

Zwei Dinge, die ohne Zusatzkosten und ohne Flow arbeiten:

**Textbausteine.** Unter „Maßnahmen" und unter „Reaktion" steht je eine
Auswahlliste. Was dort gewählt wird, hängt sich an den vorhandenen Text an –
mit Komma getrennt, mehrfach möglich. Was man auswählt, kann man nicht falsch
schreiben, und die Dokumentation wird über die Therapeuten hinweg
einheitlicher.

> Die Listen in `App.Formulas` (`tblBausteineM`, `tblBausteineR`) sind ein
> **Startvorschlag von mir** – sie stammen nicht aus Ihrer Praxis und aus
> keinem Heilmittelkatalog. Gehen Sie sie vor dem ersten Einsatz durch und
> ersetzen Sie sie durch Ihre Formulierungen. Das ist die einzige
> Pflegestelle; am Bildschirm ist nichts zu ändern.

**Längenhinweis.** Unter jedem Pflichtfeld steht still, wie viele Zeichen
schon dastehen. Unter zehn Zeichen wird der Hinweis gelb, und das Speichern
fragt nach. Das fängt „ok" und „nix" ab, ohne bei einer knappen, aber echten
Eintragung zu stören. Die Schwelle steht in `App.Formulas` unter
`minZeichen`.

**Was das ausdrücklich nicht ist:** eine Rechtschreibprüfung. Auf Tablet und
Handy läuft ohnehin die Autokorrektur der Systemtastatur. Eine echte Prüfung
per KI liegt als optionaler Zusatz in
...
- **Keine Rechtschreibprüfung.** Bausteine und Längenhinweis ja, echte
  Korrektur nein – die liegt als Zusatz in `06_Textpruefung_KI/`.
...
Die neuen Steuerelemente in V3 verwenden **ausschließlich Eigenschaften, die
in Ihrem Tenant bereits durchgelaufen sind**: Die Bausteinlisten sind
`Classic/DropDown` mit demselben Eigenschaftensatz wie `drpTherapeut`, die
Längenhinweise sind gewöhnliche `Label`. Es ist bewusst keine Eigenschaft
dazugekommen, die noch nie durch Ihr Studio gegangen ist.
