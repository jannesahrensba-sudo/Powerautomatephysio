...
> **Meldet Studio etwas an `tblBausteineM`, `tblBausteineR` oder
> `minZeichen`?** Dann fehlen die neuen benannten Formeln in `App.Formulas`.
> Sie stehen in [`../02_Eigenschaften_DE.md`](../02_Eigenschaften_DE.md)
> (bzw. `_EN.md`) am Ende des Formelblocks. `App.Formulas` muss **vor** dem
> Einfügen des Oberflächencodes gesetzt sein.
